...
\newpage
# 3 Dimensional Plots
The addition of a dimension to the PCA plots bring an increase in descriptive variability by approxiately 20%.This is not a trivial amount; however, due to the modeling of multi-dimensional plots onto two-dimensional planes, we have opted to include conclusions drawn from these additonal dimensions. However, we have included the plots here to spur further discussion for future studies.

### _3-Dimensional PCA Plot of Species_
```{r echo = false}
library(pca3d)
...
pre3d.cam <- pca3d(pca.pre, group=pre$camera)
...
post3d.cam <- pca3d(pca.post, group=post$camera)
post3d.traffic <- pca3d(pca.post, group=post$traffic)

grid.arrange(pre3d.species,post3d.species,ncol=2)
```

### _3-Dimensional PCA Plot of Mammalian Category_
```{r echo = false}
grid.arrange(pre3d.mcat,post3d.mcat,ncol=2)
```

### _3-Dimensional PCA Plot of Diel Activity_
```{r echo = false}
grid.arrange(pre3d.dnc,post3d.dnc,ncol=2)
```

### _3-Dimensional PCA Plot of Camera Preference_
```{r echo = false}
grid.arrange(pre3d.cam,post3d.cam,ncol=2)
```

### _3-Dimensional PCA Plot of Traffic Preference_
```{r echo = false}
grid.arrange(pre3d.traffic,post3d.traffic,ncol=2)
```
